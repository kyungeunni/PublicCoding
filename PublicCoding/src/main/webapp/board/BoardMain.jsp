...
<link rel="stylesheet" href="assets/css/qnaboard.css" />
</head>
		...
		<div class="major" id="boardlistmajor">
			...
			<div id="btnqst">
				<c:if test="${sessionScope.id!=null }">
					<h5>
						<a href="question.do" id="asking"><i
							class="fa fa-pencil-square-o" aria-hidden="true"></i>질문하기</a>
					</h5>
				</c:if>

				<c:if test="${sessionScope.id==null }">
					<h5>
						<a href="#" data-toggle="modal" data-target="#login" id="asking"><i
							class="fa fa-pencil-square-o" aria-hidden="true"></i>질문하기</a>
					</h5>
				</c:if>
			</div>
			...
			<div id="mainbar">
				...
				<header class="subheader">
					...
					<div id="tabs">
						<a href="?order=1" data-nav-xhref=""
							...
							<c:if test="${order==1}">class="youarehere"</c:if>> 최신등록</a> <a
							href="?order=2" data-nav-xhref=""
							title="Questions with an active bounty" data-value="featured"
							<c:if test="${order==2}">class="youarehere"</c:if>> 높은조회수 </a> <a
							href="?order=3" data-nav-xhref=""
							title="Questions with the most views, answers, and votes over the last few days"
							data-value="week"
							<c:if test="${order==3}">class="youarehere"</c:if>>인기답변 </a>
				...
				<div id="qlist-wrapper">
					<div id="question-mini-list">
						...
						<c:forEach var="d" items="${ list }">
							...
							<div class="question-summary narrow">
								...
								<div class="summary">
									<h3>
										<a href="content.do?no=${d.bno }&page=${curpage}"
											class="question-hyperlink">${d.bsubject } </a>
									</h3>
									...
									<div class="started">
										<a href="content.do?no=${d.bno }" class="started-link"><span
											title="${d.bdate }" class="relativetime">${rtime[d.bno]}</span></a>
					...
					<div class="col-sm-offset-3 col-sm-8">
						...
						<a
							href="qnaboard.do?page=${curpage>1?curpage-1:curpage }&order=${order}"><i
							class="fa fa-chevron-circle-left" aria-hidden="true">이전페이지</i></a>&nbsp;
						${curpage } page / ${totalpage } pages<a
							href="qnaboard.do?page=${curpage<totalpage?curpage+1:curpage }&order=${order}">
							<i class="fa fa-chevron-circle-right" aria-hidden="true">다음페이지</i>
						</a><br>
					...
					<ul>
						...
						<li>
							...
							data-gps-track="site.switch({ item_type:8, target_site:131 }); posts_hot_network.click({ item_type:2, location:8 })">
								Should I add redundant code now just in case it may be needed in
								the future? </a>
